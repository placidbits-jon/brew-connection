...
- Telemetry uses the fixed epoch `1789401600000` (2026-09-14 16:00 UTC). Native time-series retention is omitted so future rehearsals keep the same authored samples. Retention demonstrations belong to Phase 6.
- Embeddings come from the foundation's local `deterministic-compatibility` provider. They exercise 768-dimensional storage and indexing; they do not prove semantic relevance. The semantic candidate, keyword distractor, and Priya's favorite are seeded for Phase 5's retrieval work. Scale embeddings include repeated text to provide deterministic index volume.
